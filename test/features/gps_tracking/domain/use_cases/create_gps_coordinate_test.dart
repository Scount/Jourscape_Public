import 'package:flutter_test/flutter_test.dart';
import 'package:jourscape/features/gps_tracking/domain/repositories/gps_coordinate_repository.dart';
import 'package:jourscape/features/gps_tracking/domain/use_cases/create_gps_coordinate.dart';
import 'package:mockito/annotations.dart';
import 'package:mockito/mockito.dart';
import 'package:fpdart/fpdart.dart';
import 'package:jourscape/core/error/failures.dart';
import 'package:jourscape/features/gps_tracking/domain/entities/gps_coordinate_entitiy.dart';

import 'create_gps_coordinate_test.mocks.dart';

@GenerateMocks([GpsCoordinateRepository])
void main() {
  late CreateGPSCoordinate usecase;
  late MockGpsCoordinateRepository mockGpsCoordinateRepository;
  GpsCoordinateEntity tGpsEntity = GpsCoordinateEntity(
    id: 1,
    longitude: 10.0,
    latitude: 20.0,
    timestamp: DateTime(2023, 1, 1, 10),
  );

  setUp(() {
    mockGpsCoordinateRepository = MockGpsCoordinateRepository();
    usecase = CreateGPSCoordinate(repository: mockGpsCoordinateRepository);
    provideDummy<Either<Failure, GpsCoordinateEntity>>(Right(tGpsEntity));
  });

  group('CreateGPSCoordinate', () {
    test(
      'should call createCoordinate on the repository and return the same GpsCoordinateEntity on success',
      () async {
        when(
          mockGpsCoordinateRepository.createCoordinate(any),
        ).thenAnswer((_) async => Right(tGpsEntity));

        final result = await usecase(tGpsEntity);
        expect(result, Right(tGpsEntity));
        verify(mockGpsCoordinateRepository.createCoordinate(tGpsEntity));
        verifyNoMoreInteractions(mockGpsCoordinateRepository);
      },
    );

    test(
      'should call createCoordinate on the repository and return the same Failure on failure',
      () async {
        const failure = CacheFailure(
          message: 'Failed to create GPS coordinate in data source.',
        );
        when(
          mockGpsCoordinateRepository.createCoordinate(any),
        ).thenAnswer((_) async => const Left(failure));

        final result = await usecase(tGpsEntity);
        expect(result, const Left(failure));
        verify(mockGpsCoordinateRepository.createCoordinate(tGpsEntity));
        verifyNoMoreInteractions(mockGpsCoordinateRepository);
      },
    );
  });
}
